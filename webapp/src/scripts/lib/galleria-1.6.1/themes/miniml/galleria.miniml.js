...
;(function (window, factory) {
  ...
  Galleria.addTheme({
    ...
    init: function (options) {
      Galleria.requires(
        1.6,
        'This version of Miniml theme requires Galleria version 1.6 or later',
      )
      ...
      this.bindTooltip({
        fs: function () {
          return FULLSCREEN
            ? options._locale.exit_fullscreen
            : options._locale.enter_fullscreen
        },
      ...
      this.bind('loadfinish', function (e) {
        ...
        if (title && desc) {
          $desc
            .empty()
            .append('<strong>' + title + '</strong>', '<p>' + desc + '</p>')
          this.$('more').show()
        } else {
          this.$('more').hide()
        }
