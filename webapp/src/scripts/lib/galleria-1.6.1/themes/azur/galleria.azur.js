...
;(function (window, factory) {
  ...
  Galleria.addTheme({
    ...
    init: function (options) {
      Galleria.requires(
        1.6,
        'This version of Azur theme requires Galleria version 1.6 or later',
      )
      ...
      ;(function () {
        if (!canvSupport) {
          gallery
            ...
            .appendChild('progress', 'progressbar')
      ...
      var infoState =
          Galleria.IE < 9 ? { bottom: -100 } : { bottom: -50, opacity: 0 },
      ...
      if (options._toggleCaption) {
        ...
        this.bind('loadstart', function () {
          this.$('captionopen').toggle(
            !gallery.$('info').hasClass('open') && this.hasInfo(),
          )
        })
      ...
      if (options._showTooltip) {
        this.bindTooltip({
          fullscreen: function () {
            return gallery.isFullscreen()
              ? lang.exit_fullscreen
              : lang.enter_fullscreen
          },
          ...
          counter: function () {
            return lang.showing_image
              .replace(/\%s/, gallery.getIndex() + 1)
              .replace(/\%s/, gallery.getDataLength())
          },
        })
      }
    },
